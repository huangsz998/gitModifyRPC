...
public:
    ...
    bool is_resp_enabled() const { return resp_enabled_; }
    bool has_error() const { return !error_.get().is_nil(); }
    bool has_resp() const { return !resp_.get().is_nil(); }
    const RPCLIB_MSGPACK::object_handle& get_error() const { return error_; }
    const RPCLIB_MSGPACK::object_handle& get_resp() const { return resp_; }
